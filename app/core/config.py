...
# Phase 13 controlled WhatsApp pilot. These stay as raw strings so the pilot
# service can validate the complete bundle together and fail closed without
# preventing inbound webhook capture.
WHATSAPP_PILOT_CONFIG_ENABLED = (
    os.getenv("WHATSAPP_PILOT_CONFIG_ENABLED", "false").strip().lower() == "true"
)
WHATSAPP_PILOT_TENANT_ID = os.getenv("WHATSAPP_PILOT_TENANT_ID", "").strip()
WHATSAPP_PILOT_APPROVAL_REFERENCE = os.getenv("WHATSAPP_PILOT_APPROVAL_REFERENCE", "").strip()
WHATSAPP_PILOT_APPROVAL_EXPIRES_AT = os.getenv("WHATSAPP_PILOT_APPROVAL_EXPIRES_AT", "").strip()
WHATSAPP_PILOT_COHORT_HASHES = os.getenv("WHATSAPP_PILOT_COHORT_HASHES", "").strip()
WHATSAPP_PILOT_APPROVED_TEMPLATE_IDS = os.getenv("WHATSAPP_PILOT_APPROVED_TEMPLATE_IDS", "").strip()
WHATSAPP_PILOT_SEQUENCE_ID = os.getenv("WHATSAPP_PILOT_SEQUENCE_ID", "").strip()
WHATSAPP_PILOT_TIMEZONE = os.getenv("WHATSAPP_PILOT_TIMEZONE", "").strip()
WHATSAPP_PILOT_OPERATING_START = os.getenv("WHATSAPP_PILOT_OPERATING_START", "").strip()
WHATSAPP_PILOT_OPERATING_END = os.getenv("WHATSAPP_PILOT_OPERATING_END", "").strip()
WHATSAPP_PILOT_DAILY_CAP = os.getenv("WHATSAPP_PILOT_DAILY_CAP", "").strip()
WHATSAPP_PILOT_TOTAL_CAP = os.getenv("WHATSAPP_PILOT_TOTAL_CAP", "").strip()
WHATSAPP_PILOT_SUCCESS_MIN_DELIVERY_RATE = os.getenv("WHATSAPP_PILOT_SUCCESS_MIN_DELIVERY_RATE", "").strip()
WHATSAPP_PILOT_SUCCESS_MIN_REPLY_RATE = os.getenv("WHATSAPP_PILOT_SUCCESS_MIN_REPLY_RATE", "").strip()
WHATSAPP_PILOT_WARNING_PROVIDER_FAILURES = os.getenv("WHATSAPP_PILOT_WARNING_PROVIDER_FAILURES", "").strip()
WHATSAPP_PILOT_WARNING_QUEUE_AGE_SECONDS = os.getenv("WHATSAPP_PILOT_WARNING_QUEUE_AGE_SECONDS", "").strip()
WHATSAPP_PILOT_STOP_PROVIDER_FAILURES = os.getenv("WHATSAPP_PILOT_STOP_PROVIDER_FAILURES", "").strip()
WHATSAPP_PILOT_STOP_DEAD_LETTERS = os.getenv("WHATSAPP_PILOT_STOP_DEAD_LETTERS", "").strip()
WHATSAPP_PILOT_STOP_AI_ESCALATIONS = os.getenv("WHATSAPP_PILOT_STOP_AI_ESCALATIONS", "").strip()
WHATSAPP_PILOT_STOP_QUEUE_AGE_SECONDS = os.getenv("WHATSAPP_PILOT_STOP_QUEUE_AGE_SECONDS", "").strip()
WHATSAPP_PILOT_MAX_WORKER_HEARTBEAT_AGE_SECONDS = os.getenv("WHATSAPP_PILOT_MAX_WORKER_HEARTBEAT_AGE_SECONDS", "").strip()
